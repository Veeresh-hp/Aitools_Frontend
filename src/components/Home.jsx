...
const ProCarousel = ({
          ...
          <div className="absolute top-full mt-2 left-0 bg-gray-900/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 py-2 min-w-[220px] z-[9999] overflow-visible">
            {pricingOptions.map((option) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-white/10 transition-all duration-150 ${
                  activePricing === option.value ? 'bg-orange-500/20 border-l-4 border-orange-500' : ''
                }`}
              >
          ...
          <div className="absolute top-full mt-2 right-0 bg-gray-900/95 backdrop-blur-xl rounded-xl shadow-2xl border border-white/10 py-2 min-w-[160px] z-[9999] overflow-visible">
            {sortOptions.map((option) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-white/10 transition-all duration-150 ${
                  sortBy === option.value ? 'bg-orange-500/20 border-l-4 border-orange-500' : ''
                }`}
              >
                <span className="text-sm font-medium text-white whitespace-nowrap">
                  {option.label}
  ...
  const scrollToTop = () => {
    ...
    // No initial loading delay - render static content immediately
    ...
    const mergedToolsData = useMemo(() => {
      const dataWithApproved = toolsData.map(category => {
    ...
    const fuse = useMemo(() => {
      // Resolve constructor across different export shapes (default, named, or namespace)
      let FuseCtor = null;
      try {
      ...
      try {
        ...
        // Fallback stub to avoid runtime crashes — provides a search method returning empty array
        return { search: () => [] };
      }
    }, [toolList]);
    ...
    const filteredTools = useMemo(() => {
          ...
          tools: category.tools.filter((tool) => {
            ...
            if (activePricing !== 'all') {
              ...
              if (activePricing === 'free') {
                matchesPricing = toolPricing === 'free';
              } else if (activePricing === 'freemium') {
                matchesPricing = toolPricing === 'freemium';
              } else if (activePricing === 'paid') {
                matchesPricing = toolPricing === 'paid';
              } else if (activePricing === 'open-source') {
      ...
      if (sortBy === 'name') {
        ...
        // Sort tools by date (newest first) within each category
        result = result.map(category => ({
          ...category,
          tools: [...category.tools].filter(t => t && t.name).sort((a, b) => {
            const dateA = a.dateAdded || 0;
            const dateB = b.dateAdded || 0;
            return dateB - dateA; // Newest first (higher timestamp = more recent)
          })
        }));
      }
    ...
    // Flatten and sort once for All Tools rendering to avoid recomputation on each toggle
    ...
    useEffect(() => {
      ...
      if (!id) return;
      // If a known category hash is present, switch the active filter so the section exists
      if (id === 'all') {
        setActiveFilter('all');
    ...
    // Desktop: read ?page= from URL and clamp
    useEffect(() => {
      if (isMobile) return; // mobile ignores page param
    ...
    const categories = useMemo(() => {
      const base = [{ id: 'all', name: 'All Tools' }, ...toolsData];
      return base.map(cat => ({
        id: cat.id,
      ...
      <LazyMotion features={domAnimation}>
        <div className="min-h-screen text-white relative overflow-hidden">
          ...
          <m.div
            ...
            className={`fixed top-0 left-0 right-0 z-40 bg-gradient-to-b from-[#1a1d3a]/98 to-[#252847]/98 backdrop-blur-xl border-b border-white/10 shadow-2xl ${showStickyNav ? 'pointer-events-auto' : 'pointer-events-none'}`}
            style={{ 
          ...
          >
            ...
            >
                  ...
                  {isSearchFocused && suggestions.length > 0 && (
                    <div className="absolute top-full mt-2 w-full bg-[#1a1d3a]/95 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden z-50 border border-white/10 max-h-80 overflow-y-auto">
                      {suggestions.slice(0, 8).map((suggestion, index) => (
                        <div
                          key={index}
                          className={`px-4 py-3 cursor-pointer transition-all border-b border-white/5 last:border-b-0 ${
              ...
              <div className="py-3 overflow-x-auto scrollbar-hide">
                <div ref={stickyCatsRef} className="flex items-center gap-2 flex-wrap">
                  {(() => {
          ...
          <section
            ...
            className={`relative overflow-hidden pt-20 sm:pt-6 pb-32 sm:pb-24 z-20 min-h-[600px] -mt-4 transition-opacity duration-300 ${showStickyNav ? 'opacity-0 pointer-events-none select-none' : 'opacity-100'}`}
          ...
          >
              ...
              <div className="w-full relative z-10">
                ...
                {/* Search Bar (Hero) */}
                <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 sm:mt-12 mb-0 -mb-2 sm:mb-0">
                  <div
                  ...
                  {isSearchFocused && suggestions.length > 0 && (
                    <div className="absolute top-full mt-3 w-full bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-2xl overflow-hidden z-50 border border-gray-700/50 backdrop-blur-xl">
                      {suggestions.map((suggestion, index) => (
                        <div
                          key={index}
                          className={`px-6 py-4 cursor-pointer transition-all duration-200 border-b border-gray-700/30 last:border-b-0 ${
                            index === activeSuggestionIndex
                ...
                <div className={`flex items-center justify-between px-2 ${isMobile ? 'mb-4' : 'mb-8'}`}>
                  ...
                  <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`backdrop-blur-md text-white rounded-lg border transition-all duration-200 ${
                          isMobile ? 'p-1.5' : 'p-2'
                        } ${
